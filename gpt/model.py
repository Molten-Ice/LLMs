...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, kv_pair=None):
        ...
        if kv_pair is not None:
            cached_k, cached_v = kv_pair
            k = torch.cat([cached_k, k], dim=2).contiguous()
            v = torch.cat([cached_v, v], dim=2).contiguous()

        first_pass = kv_pair is None
        if self.flash:
            y = torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=None, dropout_p=self.dropout if self.training else 0, is_causal=first_pass)
        else:
            if first_pass:
